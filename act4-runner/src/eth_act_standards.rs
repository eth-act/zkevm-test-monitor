//! eth-act standards tests: C guests checked through their public output.
//!
//! Each ELF may have its I/O test vectors next to it: `<stem>.input` holds the
//! private input bytes (default: empty) and `<stem>.expected` holds the expected
//! public output bytes (default: the verdict `PASS`). A test passes when the
//! executor finishes cleanly and its public output matches the expected bytes.
...
/// Result of running one guest ELF against its I/O test vectors.
pub struct GuestRun {
    pub run: RunResult,
...
fn load_io_vectors(elf_path: &Path) -> Result<IoVectors> {
...
/// Run one guest ELF through `ziskemu` with its I/O test vectors.
pub fn run_zisk(ziskemu: &Path, elf_path: &Path) -> GuestRun {
    let start = Instant::now();
    let (passed, exit_code, detail) = match zisk_outcome(ziskemu, elf_path) {
...
fn zisk_outcome(ziskemu: &Path, elf_path: &Path) -> Result<(Option<i32>, Option<String>)> {
    let vectors = load_io_vectors(elf_path)?;
    let tmp = tempfile::tempdir().context("failed to create temp dir")?;
    let input_path: PathBuf = tmp.path().join("input.bin");
    let output_path: PathBuf = tmp.path().join("output.bin");
    std::fs::write(&input_path, zisk_frame_input(&vectors.input))?;
    ...
    if matches_zero_padded(&actual, &vectors.expected) {
        Ok((exit_code, None))
    } else {
        Ok((exit_code, Some(describe_mismatch(&actual, &vectors.expected))))
    }
}

/// Run one guest ELF through `sp1-eth-act-standards-executor` with its I/O test vectors.
///
...
/// the expected bytes.
pub fn run_sp1(executor: &Path, elf_path: &Path) -> GuestRun {
    let start = Instant::now();
    let (passed, exit_code, detail) = match sp1_outcome(executor, elf_path) {
        Ok((exit_code, None)) => (true, exit_code, None),
        Ok((exit_code, Some(detail))) => (false, exit_code, Some(detail)),
        Err(e) => (false, None, Some(format!("runner error: {e:#}"))),
    };
...
fn sp1_outcome(executor: &Path, elf_path: &Path) -> Result<(Option<i32>, Option<String>)> {
    let vectors = load_io_vectors(elf_path)?;
    let tmp = tempfile::tempdir().context("failed to create temp dir")?;
    let input_path: PathBuf = tmp.path().join("input.bin");
    let output_path: PathBuf = tmp.path().join("public-values.bin");
    std::fs::write(&input_path, &vectors.input)?;
    ...
    if actual == vectors.expected {
        ...
        Ok((exit_code, Some(describe_exact_mismatch(&actual, &vectors.expected))))
    }
}

/// Run one guest ELF through `openvm-eth-act-standards-executor` with its I/O test vectors.
///
...
pub fn run_openvm(executor: &Path, elf_path: &Path) -> GuestRun {
    let start = Instant::now();
    let (passed, exit_code, detail) = match openvm_outcome(executor, elf_path) {
        Ok((exit_code, None)) => (true, exit_code, None),
        Ok((exit_code, Some(detail))) => (false, exit_code, Some(detail)),
        Err(e) => (false, None, Some(format!("runner error: {e:#}"))),
    };
...
fn openvm_outcome(executor: &Path, elf_path: &Path) -> Result<(Option<i32>, Option<String>)> {
    let vectors = load_io_vectors(elf_path)?;
    let tmp = tempfile::tempdir().context("failed to create temp dir")?;
    let input_path: PathBuf = tmp.path().join("input.bin");
    let output_path: PathBuf = tmp.path().join("public-values.bin");
