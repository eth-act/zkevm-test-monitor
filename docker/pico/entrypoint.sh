...
run_act4_suite() {
    ...
    # with NOPs so Pico's transpiler doesn't panic. See patch_elfs.py for details.
    python3 /act4/patch_elfs.py "$ELF_DIR"
    echo "=== Running $ELF_COUNT tests with Pico ($CONFIG_NAME) ==="
...
run_act4_suite \
    "config/pico/pico-rv32im/test_config.yaml" \
    "pico-rv32im" \
    "I,M" \
    "$(printf 'I\nM\nZicsr\nSm')" \
    "" || true
...
run_act4_suite \
    "config/pico/pico-rv64im-zicclsm/test_config.yaml" \
    "pico-rv64im-zicclsm" \
    "I,M,Misalign" \
    "$(printf 'I\nM\nZicsr\nZicclsm\nSm\nMisalign')" \
    "-target" || true
