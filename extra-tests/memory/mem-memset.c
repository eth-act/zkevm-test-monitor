...
static int check_set(uint32_t id, size_t doff, int c, size_t len) {
    et_bytes_fill(dst, GUARD_BYTE, sizeof dst);
    et_bytes_fill(expect, GUARD_BYTE, sizeof expect);
    et_bytes_fill(expect + GUARD + doff, (uint8_t)c, len);

    void *ret = memset(dst + GUARD + doff, c, len);
    if (ret != dst + GUARD + doff || !et_bytes_eq(dst, expect, GUARD + MAX_OFFSET + len + GUARD)) {
        et_fail(id);
        return 0;
    }
    return 1;
